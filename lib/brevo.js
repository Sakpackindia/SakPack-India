...
export async function sendBrevoEmail({ to, subject, html }) {
  ...
  const senderEmail = process.env.BREVO_SENDER_EMAIL || "noreply@sakpackindia.com";
  const senderName = process.env.BREVO_SENDER_NAME || "Sakpack India";
...
function emailShell(heading, bodyHtml) {
  return `
    <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; border: 1px solid ${BRAND.border}; border-radius: 24px; background-color: ${BRAND.ink}; color: ${BRAND.ivory}; text-align: center;">
      <h1 style="color: ${BRAND.gold}; font-size: 22px; font-weight: bold; letter-spacing: 2px; margin: 0 0 24px; font-family: Georgia, serif;">SAKPACK INDIA</h1>
      ...
      <p style="color: ${BRAND.gold}; opacity: 0.7; font-size: 11px; margin: 0;">&copy; ${new Date().getFullYear()} Sakpack India. All rights reserved.</p>
    </div>
  `;
}
...
export function orderConfirmationEmailHtml({ orderNumber, customerName, items, totalAmount }) {
  const itemsHtml = (items || [])
    .map(
      (item) => `
        <tr>
          <td style="padding: 10px 0; border-bottom: 1px solid ${BRAND.border}; text-align: left; color: ${BRAND.ivory}; font-size: 13px;">${item.name} &times;${item.quantity}</td>
          <td style="padding: 10px 0; border-bottom: 1px solid ${BRAND.border}; text-align: right; color: ${BRAND.gold}; font-size: 13px; font-weight: 600;">₹${Number(item.lineTotal).toLocaleString("en-IN")}</td>
        </tr>
      `
    )
    .join("");
  ...
  return emailShell(
    "Order Confirmed!",
    `
      <p style="color: ${BRAND.ivory}; opacity: 0.75; font-size: 14px; line-height: 1.6; margin: 0 0 24px; max-width: 360px; margin-left: auto; margin-right: auto;">
      ...
      <div style="display: inline-block; font-size: 13px; font-weight: 600; letter-spacing: 1px; color: ${BRAND.gold}; padding: 10px 20px; border: 1.5px solid ${BRAND.goldDark}; border-radius: 999px; background-color: ${BRAND.cardBg}; margin-bottom: 20px;">
        Order Ref: ${orderNumber}
      </div>
      <table style="width: 100%; max-width: 360px; margin: 0 auto; border-collapse: collapse;">
        ${itemsHtml}
        <tr>
          <td style="padding: 14px 0 0; text-align: left; color: ${BRAND.ivory}; font-size: 15px; font-weight: 700;">Total</td>
          <td style="padding: 14px 0 0; text-align: right; color: ${BRAND.gold}; font-size: 15px; font-weight: 700;">₹${Number(totalAmount).toLocaleString("en-IN")}</td>
        </tr>
      </table>
      <p style="color: ${BRAND.ivory}; opacity: 0.5; font-size: 12px; line-height: 1.5; margin: 24px 0 0;">
...
export function resetPasswordEmailHtml(link) {
  return emailShell(
    ...
    `
      <p style="color: ${BRAND.ivory}; opacity: 0.75; font-size: 14px; line-height: 1.6; margin: 0 0 28px; max-width: 360px; margin-left: auto; margin-right: auto;">
        We received a request to reset your Sakpack India account password. Click the button below to choose a new one.
      </p>
