...
// couriers and assigns one automatically — so instead of a waybill you get
// an order/shipment id first, and an AWB code + assigned courier once
// Shiprocket books it with a courier.
...
// Normalized to a { ShipmentData: [{ Shipment: { Status, Scans } }] } shape
// so the admin/customer tracking UI has one consistent code path to render.
...
// GET /courier/serviceability — checks whether any courier on our Shiprocket
// account delivers to a pincode (and whether COD is available there), used
// by the storefront's pre-purchase pincode checker.
export async function checkServiceability(deliveryPincode) {
  const token = await getShiprocketToken();
  const pickupPincode = process.env.SHIPROCKET_PICKUP_PINCODE;
  if (!pickupPincode) throw new Error("Shiprocket is not configured — SHIPROCKET_PICKUP_PINCODE is missing.");

  const response = await fetchWithTimeout(
    `${API_BASE}/courier/serviceability/?pickup_postcode=${encodeURIComponent(pickupPincode)}&delivery_postcode=${encodeURIComponent(deliveryPincode)}&cod=0&weight=0.5`,
    { headers: { Authorization: `Bearer ${token}` } }
  );
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data?.message || "Shiprocket serviceability check failed.");

  const couriers = data?.data?.available_courier_companies || [];
  return {
    isServiceable: couriers.length > 0,
    codAvailable: couriers.some((c) => c.cod === 1 || c.is_cod === 1),
  };
}
