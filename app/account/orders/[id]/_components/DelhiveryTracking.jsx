  ...
  return (
    <Reveal delay={40} className="card-panel p-6 sm:p-8">
      ...
      <div className="flex flex-wrap items-center gap-3 text-lg mb-4">
        ...
        <span className="text-ink font-medium">{courierName || "Delhivery"}</span>
      ...
      {loading ? (
        ...
        <div className="space-y-2 border-t border-ink/10 pt-4">
          ...
          {liveStatus && (
            <p className="pt-2 font-display text-lg font-semibold capitalize text-gold-700">{liveStatus}</p>
          )}
        </div>
      ) : (
        cachedStatus && (
          <p className="border-t border-ink/10 pt-4 text-lg text-ink/60">
            Last known status: <span className="text-ink">{cachedStatus}</span>
          </p>
        )
      )}
    </Reveal>
