    ...
    <div className="flex flex-wrap gap-2">
      {RATING_OPTIONS.map((r) => (
        <button
          ...
          type="button"
          onClick={() => onChange(r)}
          className={`rounded-xl border px-4 py-2.5 text-base font-semibold transition-colors duration-300 ${
            Number(value) === r
              ? "border-gold-400/40 bg-gold-400/10 text-gold-700"
              : "border-gold-400/20 bg-ivory-deep/60 text-ink/60 hover:border-gold-400/30"
          }`}
        >
          {r}★
        </button>
...
function TestimonialEditForm({ testimonial, onCancel, onSaved }) {
  ...
  return (
    <form onSubmit={handleSave} className="w-full space-y-3 rounded-2xl border border-gold-400/30 bg-ink/[0.05] p-4">
      ...
      <div>
        <label className="mb-1.5 block text-base font-semibold uppercase tracking-wide text-ink/40">Rating</label>
        <RatingPicker value={form.rating} onChange={(r) => setForm((f) => ({ ...f, rating: r }))} />
      </div>
      <div className="flex items-center gap-2 pt-1">
        <button type="submit" disabled={pending} className="btn-gold flex-1 py-2.5 text-base font-semibold disabled:opacity-60">
        ...
        <button
          ...
          onClick={onCancel}
          disabled={pending}
          className="rounded-xl border border-gold-400/20 px-4 py-2.5 text-base font-semibold text-ink/60 transition-colors hover:border-gold-400/35 hover:text-ink"
        >
          Cancel
        </button>
      </div>
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,360px)]">
      <div className={`${panelClass} order-2 min-w-0 p-6 md:p-8 lg:order-1`}>
        <h2 className="mb-4 font-display text-lg font-semibold text-ink">Existing Testimonials</h2>
        {testimonials.length === 0 ? (
          <p className="py-8 text-center text-base font-semibold text-ink/40">No testimonials yet — the homepage will show default sample quotes.</p>
        ) : (
          <ul className="space-y-3">
            {testimonials.map((t) =>
              editingId === t.id ? (
                ...
                <li
                  key={t.id}
                  className="flex flex-col gap-3 rounded-2xl border border-gold-400/20 bg-ink/[0.04] p-4 sm:flex-row sm:items-center"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-base font-semibold text-ink">{t.customer_name} {t.location && <span className="text-ink/40 font-semibold">· {t.location}</span>}</p>
                    <p className="truncate text-base font-semibold text-ink/40">{t.review_text}</p>
                  </div>
                  <div className="flex shrink-0 items-center justify-between gap-3 sm:justify-start">
                    <label className="flex items-center gap-2 text-base font-semibold text-ink/60">
                    ...
                    <button
                      ...
                      disabled={pending}
                      className="rounded-xl p-2 text-ink/40 transition-colors hover:bg-gold-400/10 hover:text-gold-600 font-semibold"
                    >
                    ...
                    <button
                      ...
                      disabled={pending}
                      className="rounded-xl p-2 text-ink/40 transition-colors hover:bg-red-500/10 hover:text-red-400 font-semibold"
                    >
