          ...
          <div key={category} className={panelClass}>
            <div className="mb-5 flex items-center justify-between gap-3">
              ...
              <button
                ...
                disabled={isSaving}
                className="btn-gold px-6 py-2.5 text-base font-semibold disabled:opacity-60"
              >
                ...
                if (isToggle) {
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-4 rounded-xl border px-4 py-3 text-left text-base font-semibold transition-colors duration-300 ${
                        isOn ? "border-green-400/30 bg-green-400/10 text-green-600" : "border-ink/15 bg-ivory-deep/60 text-ink/60"
                      }`}
                    >
                      <span className="min-w-0 flex-1">
                        <span className="block font-semibold leading-snug">{TOGGLE_LABELS[item.key]}</span>
                        {item.description && (
                          <span className="mt-0.5 block text-base font-semibold leading-snug text-current opacity-70">{item.description}</span>
                        )}
                      </span>
                      <span
                        className={`relative h-5 w-9 shrink-0 rounded-full transition-colors duration-300 ${
                          isOn ? "bg-green-400/80" : "bg-ink/15"
                        }`}
                      >
                        <span
