...
const inputClass =
  "w-full rounded-xl border border-gold-400/20 bg-ivory-deep/60 px-4 py-2.5 text-sm font-semibold text-ink transition-colors duration-300 focus:border-gold-400/40 focus:outline-none focus:ring-1 focus:ring-gold-400/20 hover:border-gold-400/30";
...
export default function QuantityDiscountForm({ settings }) {
  ...
  return (
    <form
      action={formAction}
      className="max-w-xl space-y-5 rounded-[2rem] border border-gold-400/20 bg-white/85 p-6 backdrop-blur-md md:p-8"
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-display text-lg font-semibold text-ink">Cart Quantity Discount</h2>
      </div>
      ...
      <label className="flex items-center gap-3 rounded-xl border border-gold-400/20 bg-ivory-deep/60 px-4 py-3 cursor-pointer">
        ...
        />
        <span className="text-base font-semibold text-ink">Enable automatic quantity discount at checkout</span>
      </label>

      <div>
        <div className="mb-2 flex items-center justify-between">
          <label className={labelClass}>Discount Tiers</label>
          <p className="text-base font-semibold text-ink/30">Based on total items in cart (any size/variant)</p>
        </div>

        <div className="space-y-2.5">
          {tiers.map((tier, i) => (
            <div key={i} className="flex items-center gap-2.5 rounded-xl border border-ink/15 p-3">
              <div className="flex-1">
                <label className="mb-1 block text-base font-semibold uppercase tracking-wide text-ink/30">Min. Items</label>
                <input
                  type="number"
                  min={1}
              ...
              <div className="flex-1">
                <label className="mb-1 block text-base font-semibold uppercase tracking-wide text-ink/30">Discount (₹)</label>
                <input
                  type="number"
                  min={0}
                  value={tier.discount}
              ...
              <button
                ...
                className="mt-5 shrink-0 rounded-lg p-2 text-ink/40 hover:bg-ink hover:text-red-400"
        ...
        <button
          type="button"
          onClick={addTier}
          className="mt-3 flex items-center gap-1.5 text-base font-semibold text-gold-600 hover:text-gold-700"
        >
          <Plus className="h-4 w-4" /> Add Tier
        </button>
      </div>

      <p className="text-base font-semibold text-ink/30">
