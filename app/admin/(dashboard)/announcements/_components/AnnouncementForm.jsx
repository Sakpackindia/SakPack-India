...
export default function AnnouncementForm({ announcements }) {
  ...
  return (
    <div className="space-y-6">
      <form
        onSubmit={handleAdd}
        className="flex flex-col gap-3 rounded-[2rem] border border-gold-400/20 bg-white/85 p-6 backdrop-blur-md sm:flex-row sm:items-center"
      >
        ...
        <input
          ...
          className="flex-1 rounded-xl border border-gold-400/20 bg-ivory-deep/60 px-4 py-2.5 text-sm font-semibold text-ink placeholder:text-ink/30 transition-colors duration-300 focus:border-gold-400/40 focus:outline-none focus:ring-1 focus:ring-gold-400/20 hover:border-gold-400/30"
        />
        <button type="submit" disabled={pending || !message} className="btn-gold w-full shrink-0 text-sm font-semibold tracking-wider uppercase disabled:opacity-60 sm:w-auto">
          Add Announcement
        </button>
      </form>
      ...
      {announcements.length === 0 ? (
        ...
        <ul className="space-y-3">
          {announcements.map((a) => (
            ...
            >
              <p className="flex-1 text-sm font-semibold text-ink">{a.message}</p>
