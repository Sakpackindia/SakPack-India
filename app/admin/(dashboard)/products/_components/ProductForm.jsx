...
const panelClass =
  "relative rounded-[2.5rem] border border-gold-400/20 bg-gradient-to-b bg-white/95 p-6 sm:p-8 space-y-5 backdrop-blur-xl shadow-xl";
...
export default function ProductForm({ product, categories }) {
  ...
  const [gallery, setGallery] = useState(
    (product?.product_images || [])
      ...
      .map((i) => ({ url: i.image_url, color: i.color || i.variant_name || null }))
  );
  const [variants, setVariants] = useState(
    product?.product_variants?.length
      ? product.product_variants.map((v) => ({ ...v, color: v.color || "", color_hex: v.color_hex || "", price: String(v.price), original_price: v.original_price ? String(v.original_price) : "", stock_quantity: String(v.stock_quantity) }))
      : [{ variant_name: "", color: "", color_hex: "", price: "", original_price: "", stock_quantity: "" }]
  );
  ...
  return (
    <form action={formAction} onSubmit={handleSubmit} className="space-y-6">
      ...
      {(variantError || state.error) && (
        <div className="flex items-center gap-2 rounded-2xl border border-red-500/25 bg-red-500/10 p-4 text-sm font-semibold text-red-600">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <div className={panelClass}>
            <h2 className="font-display text-lg font-semibold text-ink">Basic Information</h2>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label className={labelClass}>Product Name</label>
                <input required name="name" defaultValue={product?.name} placeholder="e.g. Everyday Comfort Bra" className={inputClass} />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div>
                ...
                <input name="fabric" defaultValue={product?.fabric} placeholder="e.g. Cotton Blend" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Fit Type</label>
                <input name="fit_type" defaultValue={product?.fit_type} placeholder="e.g. Wide-Leg, Relaxed Fit" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Care Instructions</label>
                <input name="care_instructions" defaultValue={product?.care_instructions} placeholder="e.g. Machine Wash Cold" className={inputClass} />
              </div>
            </div>
            <p className="-mt-2 text-xs font-semibold text-ink/50">
              These show as separate cards in the "Specifications &amp; Fabric" section on the product page — leave any blank to skip that card.
            </p>
          ...
          <div id="variants-section" className={panelClass}>
            <h2 className="font-display text-lg font-semibold text-ink">Colors, Sizes & Pricing</h2>
            <VariantsEditor variants={variants} onChange={setVariants} showErrors={showVariantErrors} />
          </div>

          <div className={panelClass}>
            <h2 className="font-display text-lg font-semibold text-ink">FAQs</h2>
        ...
        <div className="space-y-6">
          <div className={panelClass}>
            ...
            <button
              ...
              className={`flex w-full items-center justify-between rounded-2xl border px-5 py-3.5 text-sm font-semibold transition-all duration-500 ${
                isActive ? "border-green-400/20 bg-green-500/10 text-green-600 font-semibold" : "border-gold-400/20 bg-ivory-deep/60 text-ink/50 font-semibold"
              }`}
            >
              {isActive ? "Visible in store" : "Hidden"}
              <span className={`relative h-6 w-11 shrink-0 rounded-full transition-colors duration-500 ${isActive ? "bg-green-500" : "bg-ink/10"}`}>
            ...
            <button
              ...
              className={`flex w-full items-center justify-between rounded-2xl border px-5 py-3.5 text-sm font-semibold transition-all duration-500 ${
                isFeatured ? "border-gold-400/30 bg-gold-400/10 text-gold-700 font-semibold" : "border-gold-400/20 bg-ivory-deep/60 text-ink/50 font-semibold"
              }`}
            >
              Featured on homepage
              <span className={`relative h-6 w-11 shrink-0 rounded-full transition-colors duration-500 ${isFeatured ? "bg-gold-500" : "bg-ink/10"}`}>
          ...
          <div className={panelClass}>
            <h2 className="font-display text-lg font-semibold text-ink">Featured Image</h2>
            <ImageUploader value={featuredImage} onChange={setFeaturedImage} folder="sakpack/products" />
          </div>
      ...
      <div className="flex flex-col-reverse items-center gap-4 border-t border-gold-400/20 pt-6 sm:flex-row sm:justify-between">
        <Link href="/admin/products" className="text-sm font-semibold text-ink/60 hover:text-ink">Cancel</Link>
        <button type="submit" disabled={pending} className="btn-gold w-full text-sm font-semibold tracking-widest uppercase disabled:opacity-60 sm:w-auto">
