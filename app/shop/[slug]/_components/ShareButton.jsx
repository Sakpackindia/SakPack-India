...
export default function ShareButton({ productName }) {
  ...
  const shareText = `Check out ${productName} on Sakpack India`;
  ...
  return (
    <div className="relative z-[1000]">
      <button
        ...
        className="inline-flex items-center justify-center gap-2 rounded-full border border-ink/15 bg-white px-4 py-2 text-base font-semibold uppercase tracking-[0.18em] text-ink transition-all duration-300 hover:border-ink/35 hover:scale-105"
      >
        <Share2 className="w-3.5 h-3.5 text-gold-600 animate-pulse" />
        <span>Share</span>
      </button>
      ...
      {shareOpen &&
        ...
        createPortal(
          ...
          >
            <div
              className="relative w-full max-w-[min(23rem,calc(100vw-2rem))] overflow-hidden rounded-[2rem] border border-gold-400/25 bg-gradient-to-b from-[#5c1531] via-[#4a1029] to-[#3d0d20] p-6 shadow-[0_32px_80px_rgba(0,0,0,0.95),0_0_50px_rgba(212,163,89,0.12)] transition-all duration-300 animate-[scaleUp_0.3s_cubic-bezier(0.34,1.56,0.64,1)]"
            ...
            >
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-base uppercase tracking-widest text-gold-400/60 mt-0.5">Sakpack India</p>
                </div>
                <button
                  type="button"
                  onClick={() => setShareOpen(false)}
              ...
              <p className="text-base font-semibold uppercase tracking-[0.15em] text-ivory/60 mb-4">Share via social media</p>
              ...
              <p className="text-base font-semibold uppercase tracking-[0.15em] text-ivory/60 mb-3">Or copy product link</p>
              ...
              <div className="group/copy flex items-stretch overflow-hidden rounded-2xl border border-gold-400/15 bg-ink-soft/40 transition-colors duration-300 hover:border-gold-400/35">
                <div className="flex min-w-0 flex-1 items-center gap-2 px-4 py-3 text-base text-ivory/60">
                  <LinkIcon className="h-4 w-4 shrink-0 text-gold-300" />
                  <span className="truncate pr-2 font-mono text-base text-ivory/50">{productUrl}</span>
                </div>
                <button
                  type="button"
                  onClick={copyProductLink}
                  className={`shrink-0 px-5 text-base font-semibold uppercase tracking-wider text-ink transition-all duration-300 flex items-center gap-1.5 ${
                    copied
