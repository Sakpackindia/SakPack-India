...
export default function ReviewForm({ productId, existingReview }) {
  ...
  if (done) {
    return (
      <div className="card-panel p-6 text-lg text-ink/60">
    ...
    return (
      <div className="card-panel space-y-3 p-6">
        ...
        {existingReview.review_text && (
          <p className="text-lg text-ink/70 font-light leading-relaxed">&ldquo;{existingReview.review_text}&rdquo;</p>
        )}
        {!existingReview.is_approved && (
          <p className="text-base text-ink/40">Pending approval — it will appear publicly once our team reviews it.</p>
        )}
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="card-panel space-y-4 p-6">
      ...
      <div className="flex items-center gap-1.5">
        {[1, 2, 3, 4, 5].map((n) => (
          ...
          >
            <Star
              className={`h-6 w-6 ${
                n <= (hoverRating || rating) ? "fill-gold-500 text-gold-500" : "fill-none text-ink/20"
              }`}
            />
          </button>
        ))}
      ...
      <textarea
        ...
        className="w-full rounded-xl border border-ink/15 bg-ivory-deep px-4 py-3 text-lg text-ink placeholder:text-ink/30 focus:border-gold-500/50 focus:outline-none"
      />
