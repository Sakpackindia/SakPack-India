...
function QuantityStepper({ value, onDecrease, onIncrease, increaseDisabled, className = "" }) {
  return (
    <div className={`flex items-center justify-between rounded-full border border-gold-300/40 bg-gold-400/10 p-1 ${className}`}>
      ...
      <span className="min-w-[1.5rem] text-center text-base font-semibold text-ivory">{value}</span>
      <button
        type="button"
        onClick={onIncrease}
...
function ProductModal({ product, quantity, maxQuantity, onSetQuantity, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative grid w-full max-w-2xl animate-fadeUp grid-cols-1 overflow-hidden rounded-[2rem] border border-gold-400/15 bg-gradient-to-b from-[#4a1029] via-[#3d0d20] to-[#080707] shadow-[0_30px_80px_rgba(0,0,0,0.8),0_0_50px_rgba(212,163,89,0.05)] sm:grid-cols-2">
        <button
          type="button"
          onClick={onClose}
          aria-label="Close"
          ...
          <div className="relative aspect-square shrink-0 overflow-hidden bg-ink">
            {activeImage ? (
              ...
              <div className="flex h-full items-center justify-center bg-ink-gradient">
                <HangerGlyph className="h-24 w-auto opacity-30 text-gold-300/60" />
              </div>
            )}
            {soldOut && (
              <span className="absolute left-4 top-4 rounded-full bg-red-500/20 px-3 py-1 text-base font-semibold uppercase tracking-wider text-red-300">
                Sold out
              </span>
            )}
        ...
        <div className="flex flex-col p-6 sm:p-8">
          ...
          <div className="mt-4 flex items-baseline gap-2.5">
            ...
            {hasCutPrice && <span className="text-lg text-ivory/40 line-through">₹{product.oldPrice.toLocaleString("en-IN")}</span>}
          </div>

          {product.description && <p className="mt-4 text-lg leading-relaxed text-ivory/65 font-light">{product.description}</p>}

          {!soldOut && !noRoom && (
            <div className="mt-6 flex items-center justify-between">
              <span className="text-base font-semibold uppercase tracking-widest text-ivory/40">Quantity</span>
              <QuantityStepper
                value={qtyInput}
          ...
          <button
            ...
            className={`mt-6 flex w-full items-center justify-center gap-2 rounded-full py-3.5 text-base font-semibold uppercase tracking-widest transition-all duration-300 ${
...
export default function BundleBuilder({ products, bottleCount, fixedPrice }) {
  ...
  return (
    <div className="pb-56 sm:pb-48">
            ...
            >
              <div className="relative aspect-[4/3] shrink-0 overflow-hidden bg-ink">
                {product.image ? (
                  ...
                  <div className="flex h-full items-center justify-center bg-ink-gradient">
                    <HangerGlyph className="h-16 w-auto opacity-30 text-gold-300/60" />
                  </div>
                )}
                ...
                {selected && (
                  <span className="absolute left-3.5 top-3.5 flex h-6 min-w-6 items-center justify-center rounded-full bg-gold-gradient px-1.5 text-base font-bold text-ink shadow-gold">
                    {qty}
                  </span>
                )}

                {!product.inStock && (
                  <span className="absolute left-3.5 top-3.5 rounded-full bg-red-500/20 px-3 py-1 text-base font-semibold uppercase tracking-wider text-red-300">
                    Sold out
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-1 flex-col p-2.5 sm:p-5">
                <h3 className="font-display text-base sm:text-lg text-ivory font-medium line-clamp-2 transition-colors group-hover:text-gold-100">
                  {product.name}
                </h3>
                <p className="mt-0.5 sm:mt-1 text-base sm:text-base text-ivory/40">{product.variantName}</p>
                {product.description && (
                  <p className="mt-1 hidden text-base text-ivory/50 line-clamp-2 sm:block">{product.description}</p>
                )}
                <div className="mt-auto flex items-baseline gap-1 sm:gap-2 pt-2 sm:pt-3.5">
                  <span className="font-semibold text-ivory text-base sm:text-lg">₹{product.price.toLocaleString("en-IN")}</span>
                  {product.oldPrice && product.oldPrice > product.price && (
                    <span className="text-base sm:text-base text-ivory/40 line-through">₹{product.oldPrice.toLocaleString("en-IN")}</span>
                  )}
                </div>
                ...
                {selected ? (
                  ...
                  <button
                    ...
                    disabled={disabled}
                    className={`mt-2 sm:mt-3 flex w-full items-center justify-center gap-1.5 whitespace-nowrap rounded-full py-2 sm:py-2.5 text-base sm:text-base font-semibold uppercase tracking-wide sm:tracking-widest transition-all duration-300 ${
      ...
      {/* Sticky build dock */}
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-gold-400/15 bg-[#3d0d20]/95 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] backdrop-blur-xl pb-[env(safe-area-inset-bottom)]">
        ...
        <div className="mx-auto max-w-wrap px-3 py-3 sm:px-6 sm:py-4 md:px-12">
          ...
          <div className="flex items-center justify-between gap-2 sm:gap-3">
            <p className="flex min-w-0 items-center gap-1.5 whitespace-nowrap text-base sm:gap-2 sm:text-lg font-medium text-ivory/90">
              {isComplete ? (
                <>
            ...
            <span className="shrink-0 text-base sm:text-lg font-semibold text-ivory/70">
          ...
          <div className="mt-3.5 flex flex-col gap-3 sm:mt-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div key={p.variantId} className="group relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border border-gold-400/30 bg-ink-soft sm:h-20 sm:w-20">
                  {p.image ? (
                    ...
                    <div className="flex h-full items-center justify-center">
                      <HangerGlyph className="h-8 w-auto text-gold-300/50" />
                    </div>
                  )}
                  {qty > 1 && (
                    <span className="absolute bottom-1 right-1 rounded-md bg-black/80 px-1.5 py-0.5 text-base font-bold text-ivory">
                      ×{qty}
                    </span>
                  )}
                  <button
            ...
            <div className="flex shrink-0 items-center justify-between gap-3 sm:justify-end sm:gap-4">
              {total > 0 && (
                <p className="text-left text-base text-ivory/60 sm:text-right sm:text-base">
                  <span className="hidden sm:inline">Total </span>
                  <span className={`text-lg sm:text-xl ${hasBundlePrice ? "font-semibold text-gold-300" : "font-semibold text-ivory"}`}>
                    ₹{total.toLocaleString("en-IN")}
                  </span>
                  {hasBundlePrice && (
                    <span className="ml-1 sm:ml-1.5 text-base sm:text-lg text-ivory/40 line-through">
              ...
              <button
                ...
                disabled={!isComplete}
                className="btn-gold flex flex-1 items-center justify-center gap-1.5 whitespace-nowrap px-4 py-3 text-base font-semibold tracking-wide uppercase shadow-[0_4px_20px_rgba(212,163,89,0.15)] transition-all duration-300 enabled:hover:-translate-y-0.5 enabled:hover:shadow-[0_4px_25px_rgba(212,163,89,0.3)] disabled:opacity-40 sm:flex-none sm:gap-2 sm:px-7 sm:py-3.5 sm:text-base sm:tracking-widest"
              >
