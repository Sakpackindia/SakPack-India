...
export default function PerfumeryShowcase({ products }) {
  ...
  return (
    <section className="bg-gradient-to-b from-[#3d0d20] via-[#4a1029] to-[#3d0d20] py-28 relative overflow-hidden border-t border-ink-line">
      ...
      <div className="mx-auto max-w-wrap px-6 md:px-12 relative z-10">
        ...
        <Reveal className="text-center max-w-xl mx-auto mb-16">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-gold-400/20 bg-ink-soft/80 text-base font-semibold uppercase tracking-[0.25em] text-gold-200 backdrop-blur-md mb-4">
              ...
              >
                ...
                >
                  ...
                  {p.image ? (
                    ...
                    <HangerGlyph className={`h-1/2 w-auto transition-transform group-hover:scale-105 ${isCenter ? "opacity-80 text-gold-300" : "opacity-45 text-ivory/60"
                      }`} />
                  )}
                  ...
                  <div className="absolute inset-0 bg-ink/90 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center p-3 text-center transition-opacity duration-300">
                    <span className="font-display text-base text-gold-200 leading-tight font-medium truncate max-w-full">
                      {p.name}
                    </span>
                    <span className="text-base uppercase tracking-widest text-ivory/40 mt-1">
                      View details
                    </span>
                  </div>
                </Link>
