  ...
  return (
    <div>
      <div className="flex flex-wrap gap-3">
        {urls.map((url, idx) => (
          <div key={url} className={`relative overflow-hidden rounded-xl border ${idx === 0 && showCover ? "border-gold-400/60" : "border-ink/15"} ${previewClassName}`}>
            ...
            {showCover && (
              <button
                ...
                className={`absolute bottom-1 left-1 flex items-center gap-1 rounded-full px-1.5 py-0.5 text-base font-semibold uppercase tracking-wide backdrop-blur-sm ${
          ...
          >
            {({ open }) => (
              <button
                type="button"
                onClick={() => open()}
                className={`flex flex-col items-center justify-center gap-1.5 rounded-xl border border-dashed border-ink/15 text-ink/40 transition-colors hover:border-gold-400/50 hover:text-gold-600 ${previewClassName}`}
              >
