...
export async function requestSignupOtp(_prevState, formData) {
  ...
  const emailResult = await sendBrevoEmail({
    to: email,
    subject: "Your Sakpack India Verification Code",
...
// Temporary bypass of email OTP verification — creates the account
// immediately from the signup form. Swap RegisterForm's submit back to
// requestSignupOtp/verifySignupOtp (the two-step flow above) to restore
// OTP verification later; nothing here needed to change for that.
export async function registerDirect(_prevState, formData) {
  const fullName = formData.get("full_name");
  const email = formData.get("email");
  const password = formData.get("password");
  const phone = formData.get("phone");
...
export async function requestPasswordReset(_prevState, formData) {
  ...
  const emailResult = await sendBrevoEmail({
    to: email,
    subject: "Reset your Sakpack India password",
